                            ...
                            <div class="table-responsive">
                                <table class="table" id="example">
                                    <thead>
                                     ...
                                     <tbody>
                                        <!-- Vòng lặp để hiển thị danh sách members từ cơ sở dữ liệu -->
                                        <c:forEach var="member" items="${members}">
                                            <tr>
                                                <td>${member.userId}</td>
                                                <td>${member.fullName}</td> <!-- Hiển thị fullName -->
                                                <td>${member.userName}</td> <!-- Hiển thị username (email) -->
            									<td>${member.phone != null ? member.phone : 'Chưa có số điện thoại'}</td> <!-- Hiển thị phone -->
                                                <td>${member.role}</td> <!-- Hiển thị role -->
                                                <td>
                                                    <a href="#" class="btn btn-sm btn-primary">Sửa</a>
													<form action="${pageContext.request.contextPath}/user-table"
														method="POST" style="display: inline;">
														<input type="hidden" name="userId"
															value="${member.userId}">
														<button type="submit" class="btn btn-sm btn-danger">Xóa</button>
													</form> <a href="user-details.html" class="btn btn-sm btn-info">Xem</a>
                                                </td>
                                            </tr>
                                        </c:forEach>
                                    </tbody>
